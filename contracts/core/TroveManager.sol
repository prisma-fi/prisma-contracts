...
contract TroveManager is PrismaBase, PrismaOwnable, SystemStart {
    ...
    uint256 public constant SUNSETTING_INTEREST_RATE =
        (INTEREST_PRECISION * 5000) / (10000 * SECONDS_IN_YEAR); //50%    // During bootsrap period redemptions are not allowed
    /*
     ...
     */
    function setRedemptionsCallback(
        ITroveRedemptionsCallback redemptionsCallback
    ) external onlyOwner {
     ...
     */
    function setRedemptionFeesRebate(
        uint16 _redemptionFeesRebate
    ) external onlyOwner {
    ...
    function redeemCollateral(
        ...
        require(
            block.timestamp >= systemDeploymentTime + bootstrapPeriod,
            "BOOTSTRAP_PERIOD"
        );
        ...
        uint256 userRebate = (redemptionFeesRebate * totals.collateralFee) /
            10000;
        ...
        if (userRebate > 0) {
            for (uint256 i; i < _numberOfRedemptions; ) {
                ...
                surplusBalances[refund.account] +=
                    (userRebate * refund.collateralLot) /
                    totals.totalCollateralDrawn;
                unchecked {
                    ++i;
                }
